...
class StepPhaseTracker:
    ...
    def __init__(self):
        """Initialize tracker."""
        # step -> {"cop": phase_at_step, "police": phase_at_step, ...}
    ...
    def both_at_phase(self, step: int, phase: ProtocolPhase) -> bool:
        ...
        Returns:
            True if both "cop" and "police" are at or past phase.
        """
        if step not in self.step_phases:
            return False

        phases = self.step_phases[step]
        return phases.get("cop") == phase.value and phases.get("police") == phase.value
