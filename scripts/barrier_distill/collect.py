...
def collect(seed: int, episodes: int, out: Path) -> dict:
    """Roll episodes with the search+hook teacher against the mixed thief pool."""
    import torch
    ...
    teacher = SearchHookTeacher()
    pool = make_pool()
    episodes_out, place_labels, outcomes = [], 0, {}
    t0 = time.time()
    for ep in range(episodes):
        thief_pol = pool[ep % len(pool)]
        thief_pol.reset()
        teacher.reset()
        state = _initial_state(rng, random_start=(ep % 2 == 1))
        scent = make_scent_fields(state.grid_size)
        belief = BeliefEngine(state.grid_size, "cop")  # unused under uniform belief
        feats, labels = [], []
        outcome = "steps"
        while state.turn < 35:
            legal = _legal(state, "cop")
            obs, _mask = _observation(state, "cop", scent, belief, legal, (ep % 6) + 1)
            action = teacher.action(state, legal)
            thief_action = thief_pol.action(state, rng)
            feats.append(obs)
            labels.append(COP_ACTIONS.index(action))
            place_labels += int(action.startswith("PLACE_"))
            result = apply_joint_action(state, action, thief_action)
        ...
        episodes_out.append(
            {
                ...
                "opponent": type(thief_pol).__name__ + getattr(thief_pol, "variant", ""),
                "outcome": outcome,
            }
        )
        print(
            f"[shard {seed}] ep {ep + 1}/{episodes} {episodes_out[-1]['opponent']}"
            f" -> {outcome} ({time.time() - t0:.0f}s, PLACE labels {place_labels})",
            flush=True,
        )
    torch.save({"seed": seed, "episodes": episodes_out}, out)
    return {"episodes": len(episodes_out), "place_labels": place_labels, "outcomes": outcomes}
...
def main() -> None:
    ...
    args = ap.parse_args()
    out_dir = RESULTS / "shards"
    out_dir.mkdir(parents=True, exist_ok=True)
    out = Path(args.out) if args.out else out_dir / f"shard_{args.seed:02d}.pt"
    stats = collect(args.seed, args.episodes, out)
    print(f"[shard {args.seed}] DONE {stats}", flush=True)
