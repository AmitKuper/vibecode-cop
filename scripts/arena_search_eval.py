...
class CheckpointPolicy:
    ...
    def select_action(self, obs: LocalObservation, belief, legal: list[str]) -> str:
        features = torch.tensor(local_obs_to_tensor(obs, belief, None),
...
def _obs(own, remaining, barriers, scent_grid, step):
    return LocalObservation(
        own_position=tuple(own), own_barriers_remaining=remaining,
        known_barriers=[tuple(b) for b in barriers], opponent_scent=scent_grid,
        last_hint="", step=step, gamelet=1, grid_size=N)
...
def play(cop_policy, thief_policy, seed: int, jitter: bool) -> tuple[str, int]:
    ...
    if jitter:  # break argmax determinism across games: random legal first thief move
        moves = [a for a, (dx, dy) in MOVE_DELTAS.items()
    ...
    for step in range(1, 36):
        ...
        thief_frame = [[thief_frame_wire.get(f"{r},{c}", 0.0) for c in range(N)]
                       for r in range(N)]
        # --- cop half-move (sees the thief's fresh frame) ---
        mask = compute_legal_mask_cop(tuple(cop), [tuple(b) for b in barriers],
                                      remaining, N)
...
def main() -> None:
    ...
    p.add_argument("--jitter", action="store_true",
                   help="randomise the thief's first move so games differ")
    args = p.parse_args()
    ...
    for g in range(args.games):
        ...
        steps.append(step)
    print(f"cop={args.cop} thief={args.thief} games={args.games} "
