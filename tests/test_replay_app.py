...
def _make_signed_result(tmp_path: Path, game_uid: str = "test-game-001") -> tuple[Path, bytes]:
    ...
    agreement = ResultAgreement(
        ...
        schema_version="1.0",
        gamelet_outcomes=[
            GameletOutcome(gamelet=1, cop_score=1, thief_score=0, winner="cop", turns_played=5)
        ],
        cop_total_score=1,
        thief_total_score=0,
        series_winner="cop",
...
def _make_journal(tmp_path: Path, game_uid: str, num_steps: int = 3) -> Path:
...
class TestReplayAppLoad:
    ...
    def test_load_missing_journal(self, tmp_path):
        result_path, _ = _make_signed_result(tmp_path)
        app = ReplayApp()
        ok = app.load(str(result_path), {1: str(tmp_path / "nonexistent.json")})
        # Missing journal file — StepJournal loads empty, but verify_chain on empty is ok
        # The journal just won't exist as a file, StepJournal creates empty state
        # So we check it doesn't crash; either ok or not is acceptable but no exception
        assert isinstance(ok, bool)

    def test_tampered_chain_detected(self, tmp_path):
        result_path, _ = _make_signed_result(tmp_path)
        journal_path = _make_journal(tmp_path, "test-game-001")
        # Tamper with a journal entry after creation
...
class TestReplayNavigation:
    def _loaded_app(self, tmp_path) -> ReplayApp:
        result_path, _ = _make_signed_result(tmp_path)
        journal_path = _make_journal(tmp_path, "test-game-001", num_steps=5)
        app = ReplayApp()
        app.load(str(result_path), {1: str(journal_path)})
        return app
