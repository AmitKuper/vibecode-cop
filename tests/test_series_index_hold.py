...
raise ``NoGameHappened`` (retriable at the same index, bounded by wall-clock);
...
from live_match_ref3 import NoGameHappened, _poll_agreement, _poll_turn  # noqa: E402
...
def test_no_game_happened_is_a_timeout_error() -> None:
    ...
    assert issubclass(NoGameHappened, TimeoutError)


def test_handshake_greeting_timeout_is_retriable() -> None:
    with pytest.raises(NoGameHappened):
...
def test_timeout_after_steps_exchanged_spends_the_index() -> None:
    ...
    assert not isinstance(exc_info.value, NoGameHappened)
