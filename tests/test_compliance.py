...
def test_shared_config_has_required_sections():
    ...
    assert "board_and_agents" in cfg, "Missing [board_and_agents]"
    assert "movement_and_barriers" in cfg, "Missing [movement_and_barriers]"
    assert "scoring" in cfg, "Missing [scoring]"
    assert "pheromones" in cfg, "Missing [pheromones]"
    assert "network_and_league" in cfg, "Missing [network_and_league]"
    assert "rate_limiter_gatekeeper" in cfg, "Missing [rate_limiter_gatekeeper]"
...
def test_shared_config_barrier_quota_minimum():
    ...
    assert bq >= 14, f"barrier_quota must be >= 14, got {bq}"
...
def test_shared_config_max_turns_minimum():
    ...
    assert mt >= 35, f"max_turns must be >= 35, got {mt}"
