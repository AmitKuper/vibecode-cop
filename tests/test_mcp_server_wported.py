...
def test_duplicate_start_raises() -> None:
    ...
    uid = _uid("002")
    server.start_gamelet(uid, 1, VALID_TERMS, "group-B", "thief")
...
def test_role_is_thief() -> None:
    """get_status always returns role='thief'."""
    uid = _uid("003b")
    server.start_gamelet(uid, 1, VALID_TERMS, "group-B", "thief")
    status = server.get_status(uid, 1)
...
def test_start_multiple_sub_games_same_uid() -> None:
    ...
    uid = _uid("006")
    server.start_gamelet(uid, 1, VALID_TERMS, "group-B", "thief")
    server.start_gamelet(uid, 2, VALID_TERMS, "group-B", "thief")
