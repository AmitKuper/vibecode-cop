...
| **Portability** | `uv` + committed `uv.lock`, CI runs `uv sync --frozen` + `uv lock --check` on ubuntu while development and all seven counted games ran on Windows 11 — the same lockfile serves both. Per-OS install/run instructions in `README.md`; no OS-specific paths in library code. |
