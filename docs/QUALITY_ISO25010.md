...
checkable evidence in this repository. Current as of 2026-08-23.

| Characteristic | Concrete evidence in this repo |
|---|---|
| **Functional suitability** | Ten counted league series completed and settled (`results/counted_series.json`): **7W–1L–2D, 759 points** — loss 35–75 vs anrbj666, seven **90–30 wins** (imreeyal, uoh-sqak, rstabcde, najamjad, nis-yar1, bestteam, vm__fabi), two 47–47 draws (SMNGRP05, cosmos77), 6/6 audits `Verified OK` each (`evidence/game_vs_*/`). Kit conformance: `verify_vectors.py` **125 checks / 15 fixtures ALL PASS** at kit `be96e57`; the CORE pheromone vectors are also ported verbatim into `tests/test_scent_chebyshev.py` so drift fails CI. |
...
| **Performance efficiency** | Per-move search budget: iterative deepening with cost prediction (a depth is not started unless its predicted ~10x cost fits) under an **18 s cop budget** within the signed 30 s turn deadline (raised from 10 s on 2026-08-23; the thief keeps 10 s); raw depth-4 measured up to ~74 s in the open midgame (`cop_worker/rl/pursuit_search.py`, DESIGN AD-6). Territory eval costs two 49-cell BFS maps per leaf — measured affordable. |
| **Compatibility / interoperability** | Reference-v3 wire proven against ten independent opponent implementations plus the unmodified kit's sparring peer (`scripts/verify_reference_v3_interop.py`). Unknown-peer MCP interfaces handled by the protocol-introspection pipeline (`cop_worker/protocol/`: `transport_probe → introspector → schema_mapper → mapping_plan → adapter/profile → verifier`). |
...
| **Portability** | `uv` + committed `uv.lock`, CI runs `uv sync --frozen` + `uv lock --check` on ubuntu while development and all ten counted games ran on Windows 11 — the same lockfile serves both. Per-OS install/run instructions in `README.md`; no OS-specific paths in library code. |
