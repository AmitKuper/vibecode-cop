...
| **Usability** | Live GUI (`cop_worker/gui/app.py` + `live_view_model.py`); replay tooling (`scripts/replay_viewer.py`); workspace `game-status` skill parses match logs into a human status report; `docs/counted_game_checklist.md` + workspace `MATCH_DIAGNOSIS_PLAYBOOK.md` turn log signatures into operator instructions. |
...
| **Portability** | `uv` + committed `uv.lock`, CI runs `uv sync --frozen` + `uv lock --check` on ubuntu while development and both counted games ran on Windows 11 — the same lockfile serves both. Per-OS install/run instructions in `README.md`; no OS-specific paths in library code. |
...
- **New scent model** — register the kit-recomputed lock hash in `SCENT_LOCKS`
  (`cop_worker/protocol/reference_v3.py`), implement the emission (as
...
Concurrency in the match runner is **asyncio on a single event loop** — wire I/O
...
`net_gateway`) are **per-process** — rate limits are not coordinated across
processes, which is safe here because production runs a single match-runner process
(the port preflight enforces exactly that).
