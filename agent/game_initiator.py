...
class GameInitiator:
    ...
    def __init__(
        ...
        Args:
            cop_url: Cop's MCP endpoint
            thief_url: Thief's MCP endpoint
    ...
    async def start_game(
        self, game_id: str = None, timeout_seconds: float = 30.0
    ) -> dict:
        """Start a new game between cop and thief.

        Args:
            game_id: Game identifier (auto-generated if None)
            timeout_seconds: Timeout for handshake (default 30s to allow agent startup)

        Returns:
            Dict with game_id and result
        """
        ...
        # Wait for agents to be ready (retry ping with exponential backoff)
        ...
        try:
            ...
            cop_response = await call_start_game(
                ...
                StartGameMessage(
                    ...
                    peer_url=thief_base,   # tell cop where to find thief
                ),
                "cop",
                self.secret,
            )
            ...
            logger.info("Sending handshake to thief...")
            thief_response = await call_start_game(
                ...
                StartGameMessage(
                    ...
                    endpoint=thief_base,
                    timestamp=datetime.now().isoformat(),
                    peer_url=cop_base,    # tell thief where to find cop
                ),
                "thief",
                self.secret,
            )
            ...
            logger.info("Thief accepted game")
    ...
    # Keep private aliases for any code that called them on the instance
    async def _wait_for_readiness(
        self, client: GameMCPClient, role: str, timeout_seconds: float = 10.0
    ) -> bool:
        """Delegate to module-level wait_for_readiness."""
    ...
    async def _call_start_game(
        ...
        """Delegate to module-level call_start_game."""
