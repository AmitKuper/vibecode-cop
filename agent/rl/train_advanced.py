...
def train_ppo_vs_frozen(
    ...
    agent = PPOAgent(role, grid_size=cfg.grid_size, rollout_size=rollout_size,
                     ...
                     n_actions=n_actions, n_channels=n_channels)

    cop_obs, thief_obs = env.reset()
    done = False; info: dict = {}
    winners: list = []; ep_lens: list = []
    step = update_count = 0; best_wr = -1.0
    t0 = time.time()
    ...
    while step < total_steps:
        for _ in range(rollout_size):
            ...
            cop_obs, thief_obs = next_cop, next_thief; step += 1
            if done:
                winners.append(info.get("winner")); ep_lens.append(info.get("turn", 0))
                cop_obs, thief_obs = env.reset(); done = False

        agent_obs = cop_obs if role == "cop" else thief_obs
        agent.update(agent_obs, done); update_count += 1

        if update_count % log_interval == 0 and winners:
            window = winners[-100:]
            wr = window.count(role) / len(window)
            avg_len = sum(ep_lens[-100:]) / len(ep_lens[-100:])
            logger.info(f"[PPO-frozen {role}] step={step:>7}  {role}_wr={wr:.2f}"
    ...
    while step < total_steps:
        for _ in range(rollout_size):
            ...
            if done:
                winners.append(info.get("winner")); ep_lens.append(info.get("turn", 0))
                cop_obs, thief_obs = env.reset()
                current_opp = _rng.choices(opponent_pool, weights=norm_weights)[0]; done = False
        ...
        if update_count % log_interval == 0 and winners:
            ...
            wr = window.count(role) / len(window)
            avg_len = sum(ep_lens[-100:]) / len(ep_lens[-100:])
            logger.info(f"[PPO-league {role}] step={step:>7}  {role}_wr={wr:.2f}"
                        f"  avg_len={avg_len:.1f}  updates={update_count}  elapsed={time.time()-t0:.0f}s")
            if wr > best_wr:
                best_wr = wr; agent.save(best_path)
