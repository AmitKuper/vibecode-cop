...
async def _run_single_turn(
    ...
    board_state_dict = {
        **runner._board_to_state(board), "scent_field": rules.get_scent_field()
    }
    ...
    if not rules.validate_move("cop", cop_move):
        logger.warning(f"Invalid cop move '{cop_move}' at step {step}, using STAY")
        cop_move = "STAY"
    if not rules.validate_move("thief", thief_move):
        logger.warning(f"Invalid thief move '{thief_move}' at step {step}, using STAY")
        thief_move = "STAY"
...
async def run_turn_loop(
    ...
    Args:
        watchdog_timeout: If set, each full turn (commit+reveal) must finish within
            this many seconds or the game aborts with TECHNICAL_LOSS.
    """
    winner = None
    abort_reason = None
    final_step = 0

    # Derive watchdog from shared config if not explicitly supplied
    if watchdog_timeout is None:
        try:
            from agent.config.shared_config import load_shared_config
            cfg = load_shared_config()
            watchdog_timeout = cfg.get("network_and_league", {}).get("watchdog_timeout_sec", 60)
        except Exception:
            watchdog_timeout = 60
        ...
        for step in range(1, max_turns + 1):
            ...
            try:
                outcome_tuple = await asyncio.wait_for(
                    _run_single_turn(runner, game_id, step, board, rules),
