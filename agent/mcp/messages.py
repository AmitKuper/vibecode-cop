...
from agent.mcp.messages_game import ActionMessage, StartGameMessage

logger = logging.getLogger(__name__)

# Re-export so all existing ``from agent.mcp.messages import …`` still work.
__all__ = [
    "MessagePhase",
    "MessageType",
    "StartGameMessage",
...
def validate_start_game_message(msg: StartGameMessage) -> tuple[bool, str | None]:
    ...
    if not msg.roles or "cop" not in msg.roles or "thief" not in msg.roles:
        return False, "roles must contain both 'cop' and 'thief'"
...
def validate_action_message(msg: ActionMessage) -> tuple[bool, str | None]:
    ...
    if msg.role not in ["cop", "thief", "initiator"]:
        return False, f"role must be 'cop', 'thief', or 'initiator', got {msg.role}"
